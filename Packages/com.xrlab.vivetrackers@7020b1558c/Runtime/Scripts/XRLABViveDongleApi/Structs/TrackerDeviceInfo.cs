    ...
    public class TrackerDeviceInfo
    {
        ...
        public void SaveToFile()
        {
            ...
            if (File.Exists(filename))
            {
                ...
                Log.dongleAPILogger?.WriteLine(json);
        ...
        internal void Update(byte btns, Status? status = null)
        {
            lastTimeUpdate = TotalMilis;
            if (status.HasValue)
        ...
        internal void UpdateMapState(int state)
        {
            ...
            {
                Log.dongleAPILogger?.WarningLine(".......ok we're stuck, end the map again");
                bump_map_once = true;
                stuck_on_static = 0;
            }
            ...
            if (state == ConstantsChorusdStatus.MAP_REBUILD_WAIT_FOR_STATIC)
            {
                ...
                {
                    if (IsClient)
                        Log.dongleAPILogger?.WriteLine("End the map?");
            ...
            if (state == ConstantsChorusdStatus.MAP_EXIST)
            {
                ...
                {
                    Log.dongleAPILogger?.WarningLine("ok we're stuck on EXISTS, end the map again");
                }
                stuck_on_exists += 1;
            }
            else
